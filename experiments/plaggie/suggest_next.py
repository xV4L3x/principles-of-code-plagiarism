#!/usr/bin/env python3
"""
suggest_next.py  —  Bayesian Optimisation advisor for Plaggie hyperparameter search.

Fits a Gaussian Process surrogate on observed (min_tokens, threshold, metric) →
...
Usage:
  ...
  ../results-analyzer/.venv/bin/python suggest_next.py --min-tokens-range 2 15
"""
...
RUNS_CSV = HERE / "out" / "plaggie_runs.csv"
...
PLAGGIE_METRICS = list(METRIC_ENCODE.keys())

THRESHOLD_GRID = np.round(np.arange(0.05, 0.96, 0.05), 2)

DEFAULT_MT_RANGE = (2, 15)
...
def encode(min_tokens: int, threshold: float, metric: str) -> list[float]:
    return [float(min_tokens), float(threshold), METRIC_ENCODE[metric]]


def feature_matrix(df: pd.DataFrame) -> np.ndarray:
    return np.array([
        encode(int(r.min_tokens), float(r.threshold), str(r.metric))
...
def gp_gradient(
    ...
    for other_label, other_enc in METRIC_ENCODE.items():
        if other_enc == m:
            continue
        grad[f"metric → {other_label}"] = mu([mt, t, other_enc]) - mu([mt, t, m])
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Suggest next Plaggie hyperparameter configuration via Bayesian Optimisation."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--min-tokens-range", type=int, nargs=2,
        default=list(DEFAULT_MT_RANGE), metavar=("MIN", "MAX"),
        help="min_tokens search range (default: 2 15)",
    )
    parser.add_argument(
    ...
    if not RUNS_CSV.exists():
        sys.exit(f"ERROR: {RUNS_CSV} not found — run plaggie_runner.py first.")
    ...
    print(f"  min_tokens={best_row['min_tokens']}  threshold={best_row['threshold']:.2f}  metric={best_row['metric']}")
    print(f"{'='*60}")
    ...
    print(f"  {'#':<3} {'min_tok':>8} {'thresh':>7} {'metric':<12}  "
    ...
    for rank, i in enumerate(selected_indices, 1):
        ...
        print(f"  {rank:<3} {mt:>8} {t:>7.2f} {m:<12}  "
              f"{ei[i]:>9.5f}  {mu_arr[i]:>8.4f}  ±{sigma[i]:.4f}  {note}")

    print(f"\nTo run top suggestion:")
    if selected_indices:
        mt, t, m = candidates[selected_indices[0]]
        print(f"  python plaggie_runner.py --min-tokens {mt} --threshold {t:.2f} --metric {m}")
    ...
    grad = gp_gradient(gp, scaler, best_point, mt_max)
